...
static sgx_status_t SGX_CDECL sgx_accessPD(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_accessPD_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_accessPD_t* ms = SGX_CAST(ms_accessPD_t*, pms);
	ms_accessPD_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_accessPD_t), ms, sizeof(ms_accessPD_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_outbuf = __in_ms.ms_outbuf;
	size_t _tmp_len = __in_ms.ms_len;
	size_t _len_outbuf = _tmp_len;
	char* _in_outbuf = NULL;

	CHECK_UNIQUE_POINTER(_tmp_outbuf, _len_outbuf);
	...
	if (_tmp_outbuf != NULL && _len_outbuf != 0) {
		if ( _len_outbuf % sizeof(*_tmp_outbuf) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_outbuf = (char*)malloc(_len_outbuf)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_outbuf, 0, _len_outbuf);
	}
	accessPD(_in_outbuf, _tmp_len, __in_ms.ms_i);
	if (_in_outbuf) {
		if (memcpy_verw_s(_tmp_outbuf, _len_outbuf, _in_outbuf, _len_outbuf)) {
